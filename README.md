...
_balance_ complements two adjacent libraries that handle the design-based
inference step that _balance_ does not. Together, they cover the full
non-probability → DiD → variance pipeline in Python.
...
The thin adapter `balance.interop.diff_diff` (added in v0.21) turns the
handoff into a single import. Install the optional extra:

```bash
pip install balance[did]
```

Then the canonical 3-line workflow is:

```python
from balance import Sample
from balance.interop.diff_diff import fit_did

s = Sample.from_frame(df, ...).set_target(target_df).adjust(method="ipw")
...
### Design-consistent variance (svy) — coming soon

When [svy](https://github.com/samplics-org/svy) reaches PyPI, _balance_
will ship `balance.interop.svy` for Taylor / replicate variance
estimation, completing the design-based inference pipeline. This
subsection is a placeholder that will be filled in once svy 1.0 lands;
the parent "Design-based inference" heading is stable so the README
