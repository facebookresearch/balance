...
class TestBalanceDFOutcomes(BalanceTestCase):
    ...
    def test_BalanceDFOutcomes_relative_response_rates(self) -> None:
        ...
        # pyrefly: ignore [missing-attribute]
        self.assertEqual(
            s_o.outcomes().relative_response_rates(relative_to="target"), None
        )
...
class TestBalanceDFWeights(BalanceTestCase):
    ...
    def test_BalanceDFWeights_r_indicator_rejects_empty_weights(self) -> None:
        sample = Sample.from_frame(
            pd.DataFrame({"id": pd.Series(dtype=int), "w": pd.Series(dtype=float)}),
            id_column="id",
            weight_column="w",
            standardize_types=False,
        )

        with self.assertRaisesRegex(ValueError, "requires at least one sample weight"):
...
class TestBalanceDF_asmd(BalanceTestCase):
    ...
    def test_BalanceDF_kld_with_whitespace_equivalent_formulas(self) -> None:
        sample = Sample.from_frame(
            pd.DataFrame(
                {
                    "id": (1, 2, 3, 4),
    ...
    def test_BalanceDF_kld_with_string_and_single_formula_list(self) -> None:
        sample = Sample.from_frame(
            pd.DataFrame(
                {
                    "id": (1, 2, 3, 4),
