...
class LovePlotTest(balance.testutil.BalanceTestCase):
    ...
    def test_scatter_positions_match_input_values(self) -> None:
        ...
        # ``get_offsets()`` returns ``numpy.ndarray | _SupportsArray | Buffer``
        # in newer matplotlib type stubs; ``np.asarray`` narrows it to a
        # concrete ``ndarray`` for Pyre + makes the [:, i] slicing valid.
        before_xy = np.asarray(collections[0].get_offsets())
        after_xy = np.asarray(collections[1].get_offsets())
    ...
    def test_threshold_lines_drawn_at_correct_x(self) -> None:
        ...
        # ``Line2D.get_xdata()`` returns a union including ``Buffer`` and
        # ``_SupportsArray`` in newer matplotlib stubs; ``np.asarray``
        # narrows it to a concrete ``ndarray`` so the indexing is valid.
        vline_xs: list[float] = []
        for line in ax.get_lines():
            xdata = np.asarray(line.get_xdata())
            # axvline draws a vertical line whose xdata is constant.
            if xdata.size >= 2 and float(xdata[0]) == float(xdata[-1]):
...
class EdgeCasesTest(balance.testutil.BalanceTestCase):
    ...
    def test_list_weights_with_non_default_df_index_pass(self) -> None:
        """T-NEW (Devmate regression guard) — list / numpy / RangeIndex
        Series weights paired with a ``df`` that has a non-default index
        (the common post-filter / post-groupby state) must NOT trip the
        index check. ``_as_series`` wraps list / array inputs in a
        ``pd.Series`` with a default ``RangeIndex``; the user has
        signalled positional alignment by passing a non-Series, so we
        only verify length, not index equality."""
        df = pd.DataFrame({"x": [1.0, 2.0, 3.0, 4.0]}, index=pd.Index([10, 20, 30, 40]))
        result_list = compute_asmd(df, [1.0, 2.0, 1.0, 2.0])
        self.assertIn("x", result_list.index)
        result_np = compute_asmd(df, np.array([1.0, 2.0, 1.0, 2.0]))
        self.assertIn("x", result_np.index)
        # A Series with an explicit default RangeIndex is also "positional"
        # by the same contract — the regression test covers it too.
        result_series = compute_asmd(df, pd.Series([1.0, 2.0, 1.0, 2.0]))
        self.assertIn("x", result_series.index)
