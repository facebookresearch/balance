...
class TestBalance_weights_stats(
    ...
    def test__check_weights_are_valid(self) -> None:
        ...
        # Empty weights are rejected with an explicit empty error. (The
        # previous TypeError "must be a number" only fired because pandas
        # used to default ``pd.Series([])`` to object dtype; the explicit
        # check now keeps the contract stable across pandas versions.)
        with self.assertRaisesRegex(ValueError, "weights \\(w\\) must be non-empty."):
    ...
    def test_check_weights_warns_on_all_zero(self) -> None:
        ...
        # All-NaN inputs warn with a NaN-specific message rather than the
        # generic "All weights are zero" wording (the latter would be
        # misleading for a vector that contains no zeros at all).
        with self.assertWarnsRegex(UserWarning, "weight entries are NaN"):
            self.assertEqual(
                _check_weights_series_are_valid(pd.Series([np.nan, np.nan])), None
            )

        # Nullable-dtype safety: ``pd.NA`` in a ``Float64`` series previously
        # crashed the validator with ``TypeError: boolean value of NA is
        # ambiguous`` because the negative / positive checks routed through
        # Python's built-in ``any()``. The new pandas ``.any()`` reduction
        # treats ``NA`` as ``False`` (skipna semantics) and the validator
        # falls into the same all-NaN warning path as ``np.nan`` floats.
        with self.assertWarnsRegex(UserWarning, "weight entries are NaN"):
            self.assertEqual(
                _check_weights_series_are_valid(
