...
def test_build_summary_unavailable_deff_and_outcome_means_block() -> None:
    outcome = pd.DataFrame({"self": [1.0, 2.0]}, index=["y1", "y2"])
    txt = _build_summary(
        is_adjusted=True,
        has_target=True,
        covars_asmd=pd.DataFrame(
            {"mean(asmd)": [0.1, 0.2]}, index=["self", "unadjusted"]
        ),
        covars_kld=pd.DataFrame(
            {"mean(kld)": [0.2, 0.4]}, index=["self", "unadjusted"]
        ),
...
def test_build_diagnostics_outcome_impact_and_ipw_optional_and_cbps_blocks() -> None:
    ...
    covars_asmd = pd.DataFrame(
        {"self": [0.1], "unadjusted": [0.2], "unadjusted - self": [0.1]}, index=["a"]
    )
