...
class TestUtil(
    ...
    def test_apply_na_action_to_frame_pair_drop(self) -> None:
        sample_df = pd.DataFrame(
            {"x": ["a", None], "y": [1, 2]}, index=pd.Index([10, 11])
        )
        target_df = pd.DataFrame(
            {"x": ["a", "b"], "y": [None, 3]}, index=pd.Index([20, 21])
        )
    ...
    def test_apply_na_action_to_frame_pair_add_indicator(self) -> None:
        sample_df = pd.DataFrame({"x": ["a", None]}, index=pd.Index([10, 11]))
        target_df = pd.DataFrame({"x": [None, "b"]}, index=pd.Index([20, 21]))
        sample_weights = pd.Series([1.0, 2.0], index=sample_df.index)
        target_weights = pd.Series([3.0, 4.0], index=target_df.index)
    ...
    def test_apply_na_action_to_frame_pair_add_indicator_preserves_categorical(
        ...
        sample_df = pd.DataFrame(
            {"x": pd.Series(["a", None], dtype="category")}, index=pd.Index([0, 1])
        )
        target_df = pd.DataFrame(
            {"x": pd.Series([None, "b"], dtype="category")}, index=pd.Index([0, 1])
        )
    ...
    def test_apply_na_action_to_frame_pair_preserves_duplicate_columns(self) -> None:
        sample_df = pd.DataFrame(
            [["a", None]], columns=["x", "x"], index=pd.Index([10])
        )
        target_df = pd.DataFrame(
            [[None, "b"]], columns=["x", "x"], index=pd.Index([20])
        )
    ...
    def test_apply_na_action_to_frame_pair_handles_empty_frames(self) -> None:
        sample_df = pd.DataFrame(index=pd.Index([10, 11]))
        target_df = pd.DataFrame(index=pd.Index([20]))
        sample_weights = pd.Series([1.0, 2.0], index=sample_df.index)
