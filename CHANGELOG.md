...
- **`balance.diagnostics.standalone`** — a standalone diagnostics module for already-weighted data, exposing `compute_asmd`, `compute_kish_design_effect`, `compute_kish_ess`, `compute_essp`, `compute_r_indicator`, `diagnostics_table`, and a brand-new `love_plot` function (precedent: R's `cobalt::love.plot`). Useful when weights came from outside balance (e.g. `dd.aggregate_survey()`'s second-stage design or a pre-weighted Hive table). All math primitives wrap existing balance internals from `stats_and_plots/`; no new math is duplicated. The numerical-parity test guarantees agreement with the `BalanceFrame` route to ~5e-10. Eagerly re-exported at the top of `balance/__init__.py` so users get `balance.diagnostics.compute_asmd(...)` without an explicit import.
...
- **Survey-weighted DiD tutorial.** New `tutorials/balance_diff_diff_brfss.ipynb` walks through the BRFSS-Medicaid-expansion DiD use case end-to-end: load synthetic survey microdata via `dd.generate_survey_did_data(seed=20260430)`, reweight to ACS marginals via `balance.ipw`, aggregate to a state-quarter panel via `to_panel_for_did`, fit Callaway-Sant'Anna doubly-robust DiD via `fit_did`, run HonestDiD sensitivity, build a combined diagnostic via `as_balance_diagnostic`, and contrast with the unweighted estimate. 21 cells total (12 markdown + 9 code). References Sant'Anna-Zhao 2020, Callaway-Sant'Anna 2021, Roth-Sant'Anna-Bilinski-Poe 2023, Bruns-Smith 2023, Sarig-Galili-Eilat 2023, Ghandour-Reece 2025. Self-contained and deterministic — CI re-executes via nbconvert. Committed with cleared outputs to keep diff size small; the `deploy-website.yml` workflow re-runs and bakes outputs into the rendered Docusaurus pages.
- **CI matrix entry for diff-diff integration.** The `Build and Test` workflow now exercises `parent_balance/tests/test_interop_diff_diff.py` on Python 3.11/3.12/3.13 against both the minimum supported diff-diff pin (`==3.3.0`) and the resolved-latest within the `>=3.3.0,<4` band, via new `extras` and `diff-diff-pin` matrix axes (restricted to `ubuntu-latest` to avoid runner blowup). A new `notebook-ci.yml` workflow nbconvert-executes the new tutorial on every PR that touches the notebook or interop module, and a new `diff-diff-canary.yml` workflow runs nightly at 02:00 EST against the absolute latest diff-diff release from PyPI, opening (or updating) a GitHub issue tagged `diff-diff-incompatibility` on failure. These guards catch breakage from either side — a balance refactor that perturbs the `weight_column` contract, or a diff-diff release that renames a public symbol — before downstream users hit it. The `coverage.yml` and `deploy-website.yml` workflows install with the `[did]` extra so the new tests run under coverage and the docs build can nbconvert-execute the tutorial. Internal Buck test-matrix targets (`:balance_tests_pss2`, `:balance_tests_pss3`) gain a direct `fbsource//third-party/pypi/diff-diff:diff-diff` dep so the integration tests can `import diff_diff` under buck2 inside fbcode.
## Bug Fixes
