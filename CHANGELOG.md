...
- **`balance.stats_and_plots.love_plot.love_plot`, `BalanceDFCovars.love_plot()`, and `BalanceDFCovars.plot(dist_type="love_plot")`** — visual covariate-imbalance diagnostic in the spirit of R's `cobalt::love.plot`. The primitive accepts `(before, after=None, *, xlabel="ASMD", threshold=0.1, ax=None, library="seaborn")` and now supports static matplotlib/seaborn axes, interactive Plotly figures (`library="plotly"`), and LLM-friendly ASCII output (`library="balance"`). With both series it draws the canonical before-vs-after scatter; with only `before` (the pre-adjust diagnostic case) it draws a single-series scatter with the optional threshold reference line. New options include `line=True` to connect unweighted/weighted dots and `order_by={"before", "after", "max", "alphabetical", "none"}` to control covariate ordering. The `BalanceDFCovars.love_plot(metric=...)` method pulls before/after metric off the `BalanceFrame` lineage and dispatches across `metric` ∈ {`"asmd"` (default), `"kld"`, `"emd"`, `"cvmd"`, `"ks"`}, with the threshold default resolving to the cobalt 0.1 cutoff for ASMD and to no reference line for the other metrics. Falls back to the single-series mode when no `unadjusted` view is linked (mirroring `asmd()` rather than `asmd_improvement()`), and `.plot(dist_type="love_plot", library=...)`/`.plot(plot_type="love_plot", library=...)` now route covariate views to the same diagnostic.
