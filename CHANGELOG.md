...
- **`balance.stats_and_plots.love_plot.love_plot` and `BalanceDFCovars.love_plot()`** — visual covariate-imbalance diagnostic in the spirit of R's `cobalt::love.plot`. The primitive accepts `(before, after=None, *, xlabel="ASMD", threshold=0.1, ax=None)` and returns a matplotlib `Axes`. With both series it draws the canonical before-vs-after scatter; with only `before` (the pre-adjust diagnostic case) it draws a single-series scatter with the optional threshold reference line. The `BalanceDFCovars.love_plot(metric=...)` method pulls before/after metric off the `BalanceFrame` lineage and dispatches across `metric` ∈ {`"asmd"` (default), `"kld"`, `"emd"`, `"cvmd"`, `"ks"`}, with the threshold default resolving to the cobalt 0.1 cutoff for ASMD and to no reference line for the other metrics. Falls back to the single-series mode when no `unadjusted` view is linked (mirroring `asmd()` rather than `asmd_improvement()`).
