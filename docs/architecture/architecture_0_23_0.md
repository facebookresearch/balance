...
| 2026-07-16 | **AIPW normalization contract: was deferred to phase 2 but documented at the time.** The augmentation is valid only when `w_R` and `w_T` are on the same population scale (both from `adjust()`); Hájek self-normalization gives *asymptotic* DR (ratio bias `O(1/n)`). Phase-2 `aipw()` should require `adjust()`-calibrated weights + assert `|Σw_R−Σw_T|/Σw_T < rtol`. | Records the correctness precondition so AIPW was not built on an unstated assumption. **Superseded by the 2026-08-01 decision above:** `aipw()` now enforces this contract with `rtol = 1e-6`. |
...
observed outcome (`weighted_stats.weighted_mean`, `stats_and_plots/weighted_stats.py:114` (def) / `weighted_stats.py:165` (Hájek line) —
...
| 3 | **Doubly-robust / AIPW** `μ̂_DR = μ̂_OM + Σ_R wᵢ(yᵢ − ĝ(xᵢ)) / Σ_R wᵢ` | both weights **and** the outcome model | **either** the weights **or** the outcome model is correct | **0.23.0** (`bf.aipw()`) |
...
> **Path 1 — explicit AIPW (general; shipped):** fit `ĝ` any way (e.g. on design weights), persist
> it, and combine `outcomes_hat` with the IPW weights via `bf.aipw()` / `Sample.aipw()` (a
> BalanceFrame/Sample method, **not** on the `outcomes_hat()` view). Works for *any* learner — the
...
> fit independently of the IPW weights); general DR is delivered by the explicit AIPW estimator `bf.aipw()` (#3).
...
> Plug in a weighted linear learner to recover the exact DR identity above; the explicit AIPW estimator `bf.aipw()`
...
(pre-rename historical; the key is now `"outcomes_hat"` at `sample_frame.py:172-178`, specifically `:176`):
...
- `SampleFrame.from_frame(..., outcomes_hat_columns=...)` accepts, normalizes (`str`→list) at
  `sample_frame.py:267-268`, and validates the columns exist at `sample_frame.py:394-402`.
- Predicted columns are **excluded from inferred covariates** (`sample_frame.py:431`) and
  participate in role-overlap validation (`sample_frame.py:450-466`, key at `:454`).
- `SampleFrame.outcomes_hat_columns -> list[str]` returns a copy (`sample_frame.py:565-584`).
- `_column_roles` (incl. `"predicted"`) is deep-copied (`sample_frame.py:131`) and pruned on
  column drops (`balance_frame.py:4176-4188`, the `_filter_sf` role-prune loop; the `outcomes_hat` `.get` branch at `:4181-4184`).
- Tests lock the storage contract (renamed to `test_outcomes_hat_columns*` at ~`tests/test_sample_frame.py:300-346`);
  the `p_y` fixture convention (a predicted column for outcome `y` named **`p_y`**) is retained.

What **was absent** in the pre-0.23.0 scaffolding — **all now shipped in 0.23.0** (the original "no
matches" claim no longer holds):

- `df_outcomes_hat` DataFrame accessor — **now at `sample_frame.py:739-763`** (contrast `df_outcomes` now at `sample_frame.py:714-737`).
- `_outcomes_hat_columns` protocol accessor — **now at `sample_frame.py:909-931`** (contrast `_outcome_columns` now at `sample_frame.py:886-907`).
- `outcomes_hat()` view factory + `BalanceDFOutcomesHat` view class — **shipped**: `SampleFrame.outcomes_hat()` at `sample_frame.py:1210-1241`, `BalanceFrame.outcomes_hat()` at `balance_frame.py:3468-3526`.
- The fitted-model store, learner-fitting method, and Ŷ-production path — **all shipped**: the `_outcome_model` store at `sample_frame.py:81`/`:185`; `fit_outcome_model` at `sample_frame.py:1576` & `balance_frame.py:1469`; `predict_outcomes` at `sample_frame.py:1729` & `balance_frame.py:1590`.
- On `Sample → SampleFrame` round-trips (`from_sample`, `sample_frame.py:1865-1960`) the `outcomes_hat` **columns are now carried** (read at `:1936`, passed to `_create` at `:1958`); only the fitted `_outcome_model` object itself is not carried (it defaults to `None` at `sample_frame.py:185`).
...
```
   ...
   data                weights(data=)                   predict_outcomes(on=...)
   replay engine       _compute_ipw_matrices            build_design_matrix (NOT generalized)
...
zero-filling unseen columns). IPW's `_compute_ipw_matrices` (`balance_frame.py:1999-2060`) stays
IPW-specific — it was **not** generalized; the outcome-model axis calls `build_design_matrix` directly
for both fit and replay (see [§7](#7-design-matrix-reuse-fit-vs-transfer)).
...
│ Deep-copy of new state                  │ SampleFrame.__deepcopy__  (extend, :107-153)    │
...
│ Doubly-robust estimate  μ̂_DR            │ bf.aipw() / Sample.aipw(); aipw_point_estimate  │
...
already skips `None` children (`balancedf_class.py:477-484`, def at `:397`), so `outcomes().mean()` and
...
```
 ...
 ─────────────────────────────────────────────────────────────────────────────────────
   ...
   sf.fit_outcome_model(model="auto", outcome_columns=None, variables=None, formula=None,
     ...
     │    }
     │    # NOTE — illustrative/non-exhaustive: the actual fit_outcome_model return
     │    #   (outcome_model.py:717-738) also stores fit_matrix_type, fit_weight={"column","uniform"},
     │    #   prediction_kind, calibrated, categorical_levels, learner; perf is {col:{"r2","n"}}
     │    #   (regression) or {col:{"deviance_explained","log_loss","n"}} (classifier).
 ...
 ─────────────────────────────────────────────────────────────────────────────────────
   ...
   μ̂_OM = bf.outcomes_hat().mean()         # linked: target (and self/unadj) rows; O(1)
        = weighted_mean(ŷ_T, w_T)          # Σ w_T ŷ_T / Σ w_T   (weighted_stats.py:165)
 ...
 STEP D — doubly-robust / AIPW (SHIPPED)
 ─────────────────────────────────────────────────────────────────────────────────────
   μ̂_DR = μ̂_OM  +  weighted_mean(y_R − ŷ_R, w_R)      # target g-comp + IPW-weighted residual mean
        = bf.aipw()      # augmentation runs only over responders with an observed Y (aipw.py:119-120)
...
- `na_action="drop"` is **rejected** on the projection/holdout path (`model_matrix.py:634-640`)
  ...
  **`na_action="add_indicator"`** (default). `fit_outcome_model()` **rejects** `na_action="drop"`
  via `_validate_fit_inputs` (`outcome_model.py:368`; raise at `:378-384`), mirroring how
  `fit(method="ipw")` rejects `na_action="drop"` with stored artifacts (`balance_frame.py:1238-1244`).
...
- **Native-categorical learners** (`use_model_matrix=False`, e.g. the boosting default on
  ...
  it → wrong predictions. The replay builder **accepts** stored `_outcome_model["categorical_levels"]`:
  `_restore_categorical_levels` (`outcome_model.py:257-286`) is called by `_build_replay_matrix`
  (`outcome_model.py:757`, restore at `:769-772`); fit-time capture is `_capture_categorical_levels`
  (`:240-254`, called at `:345`, stored in the model dict at `:733`).
- **`na_action="drop"` is NOT guarded on the raw path.** The existing drop-rejection lives only on
  ...
  just concatenates. So `fit_outcome_model` **rejects** `na_action="drop"` itself in
  `_validate_fit_inputs` (`outcome_model.py:368`, raise `:378-384`; mirroring
  `BalanceFrame.fit`'s IPW guard, `balance_frame.py:1238-1244`) for the boosting default.
...
calls `getattr(linked_source, self.__name)()` (`balancedf_class.py:222-387`, `getattr(self.__name)` at `:369`/`:383`), so the view's
`name` must equal the accessor method name — here `"outcomes_hat"`.

```
                          ┌──────────────────────────────┐
                          │   BalanceDFSource (Protocol)  │  balancedf_class.py:40-105 (_outcomes_hat_columns at :89-91)
                          │   + _outcomes_hat_columns (NEW)│  → pd.DataFrame | None
                          └──────────────┬───────────────┘
      implemented by ─────────────────────┼─────────────────────
        SampleFrame._outcomes_hat_columns │   BalanceFrame._outcomes_hat_columns
        = df_outcomes_hat  (NEW)          │   = delegate to _sf_sample  (cf. _outcome_columns delegate balance_frame.py:301-307; shipped _outcomes_hat_columns delegate at :316-324)
                                          ▼
                          ┌──────────────────────────────┐
                          ...
                          │       if yhat is None:   # fall back to linked sources:
                          │         yhat = union of linked outcomes_hat cols (all-NaN responder frame);
                          │         raise ONLY if no source (backing/linked) has any
                          │         (balancedf_class.py:2860-2887; gate `if not linked_hat_columns`
                          │          at :2871, raise :2873-2880)
                          │       super().__init__(yhat,  │
                          │         sample, name="outcomes_hat",
                          │         links=links)          │
                          └──────────────┬───────────────┘
                                         │  inherits: std() var_of_mean() ci_of_mean() plot()
                                         │  OVERRIDES: mean() (target-unpopulated guard
                                         │    _raise_if_target_unpopulated → super().mean();
                                         │    balancedf_class.py:2917-2971, guard :2889-2915),
                                         │    mean_with_ci() (ci_method=), summary() (scoped DR statement)
                                         ▼
...
Factory methods (mirror `outcomes()` at `balance_frame.py:3437-3466` and
`sample_frame.py:1184-1208`; the shipped `outcomes_hat()` factories are at `balance_frame.py:3468-3526`
and `sample_frame.py:1210-1241`):
...
```
  ...
  of Y over R      (unweighted; weighted=True to opt in) │
  ...
  outcomes().mean()      residual r_R = Y_R − ŷ_R        │
      │                            │                     │
      │              Σ w_R r_R / Σ w_R ─────────────────►│  (3) AIPW  bf.aipw()
      │                                                   ▼
...
matching `fit()` / `set_fitted_model()` (`balance_frame.py:1110, 1294`;
`tests/test_balance_frame.py:2793-2814`).
...
def predict_outcomes(self, *, data: SampleFrame | None = None,
                     populate: bool | None = None) -> pd.DataFrame: ...   # Ŷ from stored model; persists
...
def fit_outcome_model(self, *, target: BalanceFrame | SampleFrame | None = None,
                      inplace: bool = True, **kwargs) -> Self: ...   # model=/variables=/weighted= are passthrough **kwargs
def set_fitted_outcome_model(self, fitted: BalanceFrame | SampleFrame, *, inplace: bool = True) -> Self: ...
def predict_outcomes(self, *, on: Literal["sample","target","both"] | None = None,  # resolves to "target" when a target is set
...
#   AIPW / μ̂_DR is bf.aipw() / Sample.aipw() (a BalanceFrame/Sample method, NOT on this outcomes_hat
#     view); returns a pd.Series (μ̂_DR per outcome) delegating to outcome_models.aipw_point_estimate.
```
...
`set_fitted_model`, `test_fit_on_subset_and_apply_to_holdout` at `test_balance_frame.py:2994`, identity assert at `:3009-3012`), not a deep copy.
...
_outcome_model = {
    ...
    "na_action": "add_indicator",
    "one_hot_encoding": bool,                # hardcoded False for outcome models (the one-hot decision is carried by use_model_matrix)
    ...
    "fit_weight": {"column": str | None, "uniform": bool},  # column is None when the weight Series is unnamed; for scoped-DR summary + bootstrap refit-weighting
...
attributes **must** be initialized in `SampleFrame._create()` (`sample_frame.py:155-188`;
`_prediction_metadata={}` at `:183`, `_outcome_model=None` at `:185`) **and** copied in
`SampleFrame.__deepcopy__()` (`sample_frame.py:107-153`, new-state copy at `:134-149`) — otherwise deep copies silently lose them. The fitted
estimators are **reference-shared** (immutable post-fit), not deep-copied on every copy (`sample_frame.py:145-148`). Populating an Ŷ
...
structurally as `_sf_sample is not _sf_sample_pre_adjust`, `balance_frame.py:771`; property at `:759-771`).

**Single source of truth:** `BalanceFrame.outcome_model` / `_outcomes_hat_columns` **delegate** to
`_sf_sample` (mirroring `_outcome_columns`, `balance_frame.py:301-307`) rather than being copied by
`_sync_sampleframe_state_from_responder` — so a `Sample` (which is both classes) can't hold a stale
inherited copy. `perf` uses the **weighted-R² / weighted-deviance helpers** now in
`stats_and_plots/weighted_stats.py`: `weighted_r2` at `weighted_stats.py:384` (used for regressor perf
at `outcome_model.py:708`), and classifier deviance via `_classifier_perf` (`outcome_model.py:289-313`)
reusing `ipw._compute_deviance` (`ipw.model_coefs` still covers only linear coefficients). The earlier
prescription — that the DR flag in `summary()` verify a linear learner has `fit_intercept=True` before
claiming double robustness — was **not implemented and is now moot**: the shipped `summary()` prints the
actual μ̂_DR (AIPW) value ungated and makes no WLS "doubly robust w.r.t." claim to guard.
...
- `stats_and_plots/weighted_stats.py` — a **weighted-R²** helper (`weighted_r2`, `:384`) for regressor `perf` (balance's first regression metric); classifier deviance-explained lives in `outcome_models/outcome_model.py::_classifier_perf` (`:289-313`) via `ipw._compute_deviance`.
...
> **Note (as executed):** the stack deviated slightly from this plan — the design doc (this base diff) and the end-to-end tutorial (the top-of-stack diff) each ship as their own standalone diff, rather than being folded into the functional diffs. Beyond the planned nine, **two additional diffs shipped**: `[balance] Add doubly-robust AIPW estimate bf.aipw() + R oracles` (D112679814) and `[balance] summary(): show IPW/OM/DR estimates when an outcome model is fit` (D112679813).
...
4. **`[balance] Add weighted R² to weighted_stats`** — reusable stats primitive.
   - *Example* (→ summary; asserted by a test; mirrored in the docstring):
     ```python
     ...
     ```
   - *Scope:* `weighted_r2` in `stats_and_plots/weighted_stats.py`, from existing `weighted_mean`/`weighted_var`.
   ...
   - *Docs:* `CHANGELOG.md` (`weighted_r2`); docstrings with examples; `ARCHITECTURE.md` supporting-modules note if applicable.
...
**`summary()` outcome-estimate trio (shipped, D112679813).** `BalanceFrame.summary()` now renders an
"Outcome estimates" section showing the μ̂_IPW / μ̂_OM / μ̂_DR trio — μ̂_IPW with a CI via
`outcomes().mean_with_ci()`, μ̂_OM and μ̂_DR as point estimates — whenever an outcome model + a target
are present. It is implemented by `BalanceFrame._outcome_estimates_summary()` (`balance_frame.py:3725`)
threaded through `summary_utils._build_summary(outcome_estimates=...)` (`summary_utils.py:177`, rendered
`~322-323`), and is distinct from the per-view `outcomes_hat().summary()` estimator line.
