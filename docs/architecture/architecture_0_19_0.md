...
**38 files changed, 11259 insertions(+), 2843 deletions(-)** across the full stack.
...
├──────────────────────────────┼─────────┼─────────┼───────────────────────────────────────┤
│ sample_class.py              │ 2383    │  237    │ Gutted: all logic moved to base       │
│                              │         │         │ classes. Now thin facade with          │
│                              │         │         │ __new__, from_frame, deepcopy,         │
│                              │         │         │ backward-compat aliases only.          │
├──────────────────────────────┼─────────┼─────────┼───────────────────────────────────────┤
│ balance_frame.py             │    0    │ 1882    │ NEW file. Adjustment orchestrator:     │
...
├──────────────────────────────┼─────────┼─────────┼───────────────────────────────────────┤
│ sample_frame.py              │    0    │ 1208    │ NEW file. DataFrame container with     │
│                              │ (new)   │         │ _column_roles dict, explicit           │
│                              │         │         │ covar_columns param, column-role        │
│                              │         │         │ metadata and BalanceDFSource protocol.  │
├──────────────────────────────┼─────────┼─────────┼───────────────────────────────────────┤
│ balancedf_class.py           │ 2987    │ 3310    │ 451 lines changed. Protocol member     │
...
| File | Changes |
|------|---------|
| `__init__.py` | Version bump to 0.19.0 (14 lines) |
...
| File | Changes |
|------|---------|
| `test_balance_frame.py` | **NEW** — 2151-line BalanceFrame-level tests |
| `test_sample_frame.py` | **NEW** — 1123-line SampleFrame tests |
| `test_e2e_from_tutorials.py` | **NEW** — 1061-line end-to-end regression tests |
| `test_balancedf.py` | 897 lines changed — BalanceDF protocol + links_override tests |
| `test_sample.py` | 406 lines changed — updated for new architecture |
...
| `CHANGELOG.md` | Rewritten for 0.19.0 release |
...
| `tutorials/balance_quickstart.ipynb` | Minor update (12 lines) |
...
│  │ .set_weights()→ own impl that preserves shared _sf_sample/     │  │
│  │                  _sf_sample_pre_adjust identity (no deep copy) │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Own properties:                                                     │
│  ┌────────────────────────────────────────────────────────────────┐  │
...
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Linking methods (return new instances via deepcopy):                 │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │ .set_target(target)     → new BF with _links["target"]        │  │
...
│  │       deepcopy _sf_sample                                     │  │
│  │       if first adj: freeze original as "weight_pre_adjust"    │  │
│  │       find next N for "weight_adjusted_N"                     │  │
│  │       add_weight_column("weight_adjusted_N", result["weight"])│  │
│  │       overwrite original weight col with adjusted values      │  │
│  │       set_active_weight(original_weight_name)                 │  │
│  │       store _adjustment_model                                 │  │
│  │       _links["unadjusted"] = original self                    │  │
...
│           │   → _build_adjusted_frame(result, method):                     │
│           │       deepcopy _sf_sample                                      │
│           │       if 1st adj: freeze original as "weight_pre_adjust"      │
│           │       find next N for "weight_adjusted_N"                      │
│           │       add_weight_column("weight_adjusted_N", result["weight"])│
│           │       overwrite original weight col with adjusted values       │
│           │       set_active_weight(original_weight_name)                  │
│           │       new._links["unadjusted"] = original_self                │
...
└──────────────────────┘  └───────────────────────┘  └──────────────────────┘
       ...
       (directly)               (own + delegation)         (via inheritance)
```
...
Sample.from_frame(df)                    Sample.from_frame(target_df)
  ...
  sample                                            target
    ...
    │   2. if 1st adj: freeze original weights as       │
    │      "weight_pre_adjust"                          │
    │   3. find next N for "weight_adjusted_N"          │
    │   4. add_weight_column("weight_adjusted_N",       │
    │      result["weight"])                            │
    │   5. overwrite original weight col with adjusted  │
    │      values; set_active_weight(original name)     │
    │   6. _create(sample=new_sf, sf_target=target)    │
    │   7. _sf_sample_pre_adjust = original pre_adjust │
    │   8. _links["unadjusted"] = original self         │
...
__new__(cls, responders=None, target=None)
    ...
    │     │
    │     ├── caller in {"__deepcopy__", "__newobj__", "__newobj_ex__",
    │     │               "__reduce__", "__reduce_ex__",
    │     │               "deepcopy", "_reconstruct", "from_frame", "_create"}
...
Purpose: Forces users to use Sample.from_frame() factory method.
         Internal paths (deepcopy, pickle via __reduce__/__reduce_ex__,
         _create) are whitelisted.
```
...
**Sample backward-compat aliases** (sample_class.py, lines 161-178):
...
**Large-target diagnostic warning** (balance_frame.py, lines 789-805):
...
**High-cardinality feature detection** (balance_frame.py, lines 807-837):
...
- `add_weight_column(name, values, metadata)` — adds a new weight column
  to `_df` and registers it in `_column_roles["weights"]` (lines 1041-1100)
- `set_active_weight(column_name)` — switches which weight column is
  considered "active" (returned by `df_weights`) (lines 976-1003)
...
- `set_weight_metadata(column, metadata)` — stores arbitrary provenance
  metadata for a weight column (lines 921-951)
- `weight_metadata(column)` — retrieves metadata for a weight column
  (lines 953-974)

**Unified weight history tracking** (`_build_adjusted_frame`, balance_frame.py line 576):
...
`set_active_weight(original_weight_name)` after overwriting the values).
