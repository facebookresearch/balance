...
├──────────────────────────────┼─────────┼─────────┼───────────────────────────────────────┤
│ sample_frame.py              │    0    │ 1373    │ NEW file. DataFrame container with     │
...
- `add_weight_column(name, values, metadata)` — adds a new weight column
  to `_df` and registers it in `_column_roles["weights"]` (lines 1201+)
- `set_active_weight(column_name)` — switches which weight column is
  considered "active" (returned by `df_weights`) (lines 1136-1163)
...
- `set_weight_metadata(column, metadata)` — stores arbitrary provenance
  metadata for a weight column (lines 1081-1111)
