...
class BalanceDFCovars(BalanceDF):
    ...
    def plot(
        ...
        if dist_type in ("love", "love_plot"):
            ...
            # ``plot_it`` is a distribution-plot kwarg (see plot_dist); for
            # love plots the equivalent is ``show`` (only meaningful for
            # ``library="plotly"``). Translate to avoid passing ``plot_it``
            # through to Plotly's ``update_layout`` where it is an invalid
            # property.
            if "plot_it" in kwargs:
                plot_it = kwargs.pop("plot_it")
                library = kwargs.get("library", "plotly")
                if library == "plotly":
                    kwargs.setdefault("show", plot_it)
                else:
                    logger.warning(
                        "plot_it is only meaningful for library='plotly' "
                        "love plots; ignoring for library=%r.",
                        library,
                    )
