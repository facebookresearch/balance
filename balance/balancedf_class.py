...
class BalanceDFOutcomes(BalanceDF):
    def __init__(
        ...
        if outcome_df is None:
            ...
            raise ValueError(
                f"Cannot create BalanceDFOutcomes: no outcome columns are defined "
                f"on the provided {source_type}. Configure the underlying object "
                "with outcome_columns= before constructing or accessing outcomes."
            )
...
class BalanceDFCovars(BalanceDF):
    ...
    def love_plot(
        ...
        *,
        ...
        ax: Any | None = None,
        library: _love_plot_module.LovePlotLibrary = "seaborn",
        line: bool = False,
        order_by: _love_plot_module.LovePlotOrderBy = "before",
        show: bool = False,
        **kwargs: Any,
    ) -> Any:
        ...
        Args:
            ...
            ax (Any | None, optional): Optional matplotlib ``Axes`` to draw
                into for static output. If ``None``, a new figure is created
                sized to the number of covariates.
            library (str, optional): Plotting backend. ``"seaborn"``/
                ``"matplotlib"`` returns a matplotlib ``Axes`` (the legacy
                behaviour), ``"plotly"`` returns an interactive Plotly
                ``Figure``, and ``"balance"`` returns an ASCII string.
            ...
            order_by (str, optional): Sort covariates by ``"before"``
                (default), ``"after"``, ``"max"``, ``"alphabetical"``, or
                ``"none"``.
            show (bool, optional): For ``library="plotly"``, whether to call
                ``fig.show()`` before returning the figure.
            **kwargs: Additional Plotly layout options when
                ``library="plotly"``.

        Returns:
            matplotlib.axes.Axes | plotly.graph_objects.Figure | str: Static
            axes for ``library="seaborn"``/``"matplotlib"``, a Plotly figure
            for ``library="plotly"``, or ASCII text for ``library="balance"``.
            The ``Any`` return annotation matches balance's plotting-helper
            convention and avoids eager plotting imports in this module's
            type signature.
    ...
    def plot(
        ...
        (or ``plot_type="love_plot"``) dispatches to :meth:`love_plot`, so
        calls such as ``adjusted.covars().plot(dist_type="love_plot",
...
class BalanceDFWeights(BalanceDF):
    ...
    def r_indicator(
        self: "BalanceDFWeights",
        target_propensity: float | npt.ArrayLike | None = None,
    ) -> np.float64:
        """Approximate the R-indicator using inverse weights as propensities.

        This is a convenience wrapper around
        :func:`weighted_comparisons_stats.r_indicator`. Sample response
        propensities are approximated from the inverse of the current weights.
        If these inverse weights exceed 1, they are rescaled by their maximum
        so the derived propensities remain in the valid ``[0, 1]`` interval.

        When ``target_propensity`` is omitted, the method requires the sample
        to have a linked target and assumes every target unit has propensity 1.
        ...
        .. code-block:: python
            ...
            sample = Sample.from_frame(
                pd.DataFrame({"id": [1, 2], "weight": [2.0, 4.0]}),
                id_column="id",
                weight_column="weight",
                standardize_types=False,
            )
            target = Sample.from_frame(
                pd.DataFrame({"id": [10, 11, 12], "weight": [1.0, 1.0, 1.0]}),
                id_column="id",
                weight_column="weight",
                standardize_types=False,
            )
        ...
        if (sample_weights <= 0).any():
            raise ValueError(
                "BalanceDFWeights.r_indicator requires strictly positive weights"
            )

        sample_propensity = np.reciprocal(sample_weights.to_numpy(dtype=float))
        max_propensity = sample_propensity.max()
