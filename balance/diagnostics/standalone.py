...
def diagnostics_table(
    ...
    *,
    ...
    Args:
        ...
        r_indicator: If True and ``target_df`` is provided, include
            ``r_indicator`` row.
    ...
    rows: dict[str, float] = {}
    if kish:
        deff = compute_kish_design_effect(weights)
        rows["deff"] = deff
        rows["ess"] = compute_kish_ess(weights)
    if essp:
        rows["essp"] = compute_essp(weights)
    if asmd:
        a = compute_asmd(df, weights, target_df, target_weights)
        for k, v in a.items():
            rows[f"asmd_{k}"] = float(v)
    if r_indicator and target_df is not None:
        # Forward target_df so the R-indicator's target propensity vector
        # has length ``len(target_df)`` — matches ``BalanceDFWeights.
        # r_indicator()`` at ``balancedf_class.py:3108`` and is required for
        # numerical parity when sample and target sizes differ.
