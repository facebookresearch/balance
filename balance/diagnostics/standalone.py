...
def _as_series(weights: list[float] | pd.Series | npt.NDArray) -> pd.Series:
    ...
    Raises:
        ValueError: If ``weights`` is empty, non-finite, has any negative
            entry, or is a multi-dimensional array. ``balance``'s upstream
            weight-stats helpers reject non-1D arrays (a 2D numeric array
            becomes object dtype when wrapped in a Series), so this
            standalone diagnostics surface enforces the same shape contract
            instead of silently flattening a misshaped input.
    """
