...
def _check_aligned(df: pd.DataFrame, weights: pd.Series) -> None:
    ...
    Length match is always required. Index match is required ONLY when
    ``weights`` carries a non-default index (i.e. the user explicitly
    passed a ``pd.Series`` with their own index labels). When
    ``weights.index`` is a default ``RangeIndex(0, len(weights))`` we
    skip the index-equality check: ``_as_series`` converts list / numpy
    inputs to that shape, and rejecting them just because ``df.index``
    happens to be non-default (e.g. post-filter, post-groupby) would
    spuriously block a common usage that users explicitly opt into via
    a list/array argument.

    NOTE: this is **strictly more permissive** than
    ``balance.utils.input_validation._check_weighting_methods_input``,
    which requires ``df.index.equals(weights.index)`` unconditionally
    (no default-RangeIndex skip). The standalone-diagnostics surface is
    a different contract: it accepts list / numpy arrays as a
    user-positional opt-in, whereas the BalanceFrame internal helper
    only sees pre-validated Series with index-aligned weights. Do not
    point users at the BalanceFrame helper as a shared spec — the two
    contracts diverge on purpose.

    Raises:
        ValueError: If ``len(df) != len(weights)``, or if ``weights``
            carries a non-default index AND that index does not equal
            ``df.index``.
    """
    ...
    # ``RangeIndex(start=0, step=1, stop=N)`` is the shape ``_as_series``
    # gives to a list / numpy / non-Series input. If the weights index
    # matches that shape, treat the weights as positional and skip the
    # index-equality check — the alternative (always check) regresses
    # the common case where ``df`` has a non-default index after
    # filtering / groupby and the user passes a plain ``[w1, w2, ...]``.
    weights_index = weights.index
    is_default_range = (
        isinstance(weights_index, pd.RangeIndex)
        and weights_index.start == 0
        and weights_index.step == 1
        and weights_index.stop == len(weights)
    )
    if is_default_range:
        return
    if not df.index.equals(weights_index):
        raise ValueError(
            ...
            "intentional and matches df by position."
        )
