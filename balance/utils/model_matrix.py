...
def _build_projected_model_matrix(
    ...
    *,
    ...
    penalty_factor_expanded = list(
        model_matrix_out.get(
            "penalty_factor", [1.0] * len(project_to_columns)
        )  # pyre-ignore[6]
    )
...
def build_design_matrix(
    ...
    *,
    ...
    matrix_type:
        Desired output type: ``"sparse"`` (:class:`csc_matrix`),
        ``"dense"`` (:class:`numpy.ndarray`), or ``"dataframe"``
        (:class:`pandas.DataFrame`).  ``None`` leaves the matrix as-is.
