...
def _check_weights_series_are_valid(
    ...
    *,
    ...
    Warns:
        UserWarning: If ``require_positive`` is False and all weights are zero
            (or non-positive). Weighted statistics over an all-zero weight
            vector silently produce ``NaN`` / ``inf`` (``sum(w*x)/sum(w) =
            0/0``). The warning surfaces the failure mode without changing
            the historical not-a-raise behaviour for the
    ...
    if not any(w > 0):
        ...
        warnings.warn(
            "All weights are zero (or non-positive); weighted statistics "
            "downstream will yield NaN or inf. This usually indicates a bug "
