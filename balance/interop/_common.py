...
def drop_history_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sample.
    """
    # Filter on ``isinstance(c, str)`` so the ``list[str]`` annotation is
    # sound under stricter pandas stubs: ``df.columns`` may carry non-string
    # labels (int, tuple, etc.), but balance-emitted history column names
    # are always strings, so they are a strict subset of the str-typed
    # columns and the predicate cannot reject any genuine history column.
    history: list[str] = [
        c for c in df.columns if isinstance(c, str) and _is_balance_history_column(c)
    ]
    if history:
...
def validate_nonzero_weights(df: pd.DataFrame, weight_col: str, *, ctx: str) -> None:
    ...
    weights: pd.Series = df[weight_col]
    # Reject non-numeric dtypes up front: a string / object weight column
    # would otherwise trip on ``(weights < 0)`` with a cryptic pandas
    # comparison TypeError, hiding the real issue (the user pointed at the
    # wrong column). Mirror the wording of ``weights_stats``'
    # ``_check_weights_series_are_valid`` so the surface stays consistent
    # across the two validators.
    if not pd.api.types.is_numeric_dtype(weights):
        raise TypeError(
            f"{ctx}: active weight column {weight_col!r} must be numeric, "
            f"got dtype {weights.dtype}. (Almost always: the column named "
            "in Sample.weight_column was an id / category column rather "
            "than a numeric weight.)"
        )
    # Reject ANY negative entry first (a mixed-sign vector like [-1, 2]
    # would pass the "at least one positive" check below but is just as
    # broken as an all-non-positive vector — the SurveyDesign math
    # assumes non-negative design weights, and a negative entry
    # almost always means the user accidentally pointed at a residual /
    # centred / mean-deviation column instead of the design-weight one).
    n_negative: int = int((weights < 0).sum())
    if n_negative > 0:
        raise ValueError(
            f"{ctx}: active weight column {weight_col!r} has "
            f"{n_negative} negative entr{'y' if n_negative == 1 else 'ies'} "
            f"({len(weights)} rows total). Design weights must be "
            "non-negative — a negative entry usually means the column "
            f"named in {weight_col!r} is actually a residual / centred / "
            "mean-deviation vector, not a design weight. Verify the "
            "column name and re-fit / re-derive the design weights "
            "before handing them to the adapter."
        )
    # Then reject the all-non-positive case (sum_w == 0).
    if not (weights > 0).any():
        raise ValueError(
            ...
            "Diagnose the upstream cause: confirm that "
            f"{weight_col!r} names the intended design-weight column "
            "(not a centred / residual column whose values can legitimately "
            "be zero or negative); inspect any filtering / merging / "
            "groupby / transformation step that may have collapsed the "
            "frame to an all-zero subset; verify the input weights "
            "themselves are positive before they reach this seam. "
            "(Note: `transformations='trim'` only clips extreme positive "
            "weights — it does not rescue an all-non-positive vector.)"
        )
