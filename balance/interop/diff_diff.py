...
* ``weight_type="pweight"`` is the documented default. It is the only
  choice compatible with the staggered DiD estimators that this adapter
  is most often used with — CallawaySantAnna, StackedDiD, ImputationDiD,
  HeterogeneousAdoptionDiD (HAD), TwoStageDiD, WooldridgeDiD, TROP,
  StaggeredTripleDifference, ChaisemartinDHaultfoeuille (dCDH),
  TripleDifference, and SyntheticDiD — each of which inlines its own
  ``weight_type != "pweight"`` rejection in its ``fit()`` method (e.g.
  CallawaySantAnna at ``staggered.py:1500-1505``). EfficientDiD,
  DifferenceInDifferences, TwoWayFixedEffects, MultiPeriodDiD,
  SunAbraham, and ContinuousDiD additionally accept ``aweight``, but
  balance does not produce precision weights, so exposing that mode
...
#: via ``design_columns``. Source of truth: the ``SurveyDesign`` dataclass
#: definition in diff-diff's ``survey.py`` (no ``ssu`` field exists there;
#: the future ``balance.interop.svy`` adapter is the home for ``ssu``).
...
def _require_diff_diff() -> ModuleType:
    ...
    call site. We use ``_assert_type`` rather than a bare ``assert``
    because ``python -O`` strips ``assert`` statements; the bare-assert
    version would leave ``_dd`` as ``None`` in optimised builds and the
    next attribute access would raise ``AttributeError`` instead of the
    rewritten ``ImportError``.
    """
    if _IMPORT_ERROR is not None:
        raise _IMPORT_ERROR
    return _assert_type(_dd, ModuleType)
...
def to_survey_design(
    ...
    *,
    ...
    design_columns :
        ...
        (``strata``, ``psu``, ``fpc``, ``replicate_weights``,
    ...
    weight_type :
        ...
        is REQUIRED for the staggered family (CallawaySantAnna, StackedDiD,
        HeterogeneousAdoptionDiD, ImputationDiD, TwoStageDiD,
        WooldridgeDiD, TROP, StaggeredTripleDifference, dCDH,
        TripleDifference, SyntheticDiD); each of those estimators inlines
        its own ``weight_type != "pweight"`` rejection in ``fit()`` (e.g.
        CallawaySantAnna at ``staggered.py:1500-1505``). EfficientDiD,
        DifferenceInDifferences, TwoWayFixedEffects, MultiPeriodDiD,
        SunAbraham, and ContinuousDiD also accept ``aweight``.
    lonely_psu :
...
def to_panel_for_did(
    ...
    *,
    ...
    # All-zero / all-negative weights would produce undefined variance in
    # the downstream SurveyDesign. Catch at the seam with an actionable
    # error rather than letting inf / NaN surface deep in aggregate_survey.
    validate_nonzero_weights(df, weights_col, ctx="to_panel_for_did")
...
def fit_did(
    ...
    **estimator_kwargs :
    ...
    # Reject reserved positional / wired-in fit kwargs before they can collide
    # with the positional ``df`` (passed as the first argument to ``fit``)
    # this adapter wires up itself. Without this guard, a caller who passed
    # ``data=other_df`` via ``**estimator_kwargs`` would land it in
    # ``fit_kwargs`` and the ``instance.fit(df, **common_fit_kwargs)`` call
    # below would raise the cryptic ``TypeError: multiple values for argument
    # 'data'`` rather than a clear adapter-level message.
    #
    # Limited to the names that genuinely flow through ``**estimator_kwargs``:
    # ``data`` / ``df`` (positional fit() conflict) and ``first_treat`` (the
    # diff-diff column name; fit_did exposes the same column via the
    # ``treatment_first=`` adapter param). Names already in fit_did's
    # explicit signature (``outcome``, ``time``, ``unit``, ``treatment``,
    # ``covariates``) bind to those params during the user's call and never
    # reach ``**estimator_kwargs``; Python rejects same-name duplication
    # with its own clear ``TypeError`` before fit_did's body runs, so an
    # adapter-level guard for those names would be dead code.
    # ``survey_design`` has a separate, more specific check below.
    _reserved_fit_positionals: frozenset[str] = frozenset({"data", "df", "first_treat"})
    _reserved_in_user: set[str] = set(estimator_kwargs) & _reserved_fit_positionals
    if _reserved_in_user:
        raise TypeError(
            f"fit_did(): the following kwarg(s) collide with positional / "
            f"wired-in arguments and must be passed via the dedicated "
            f"adapter parameters instead of `**estimator_kwargs`: "
            f"{sorted(_reserved_in_user)}. The DataFrame is sourced from "
            "`sample.df` and cannot be overridden via `data=`/`df=`. "
            "Use `treatment_first=` on fit_did() directly instead of "
            "the diff-diff `first_treat` column-name kwarg."
        )
    ...
    # Same all-zero / all-negative guard as ``to_panel_for_did``. The check
    # runs AFTER NaN-row removal so the count in the (potential) error
    # message reflects the post-NaN-filter row population the estimator
    # would actually consume.
    validate_nonzero_weights(pre_history_df, weights_col, ctx="fit_did")
