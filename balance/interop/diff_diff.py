...
def _resolve_design_columns(
    ...
    # Validate user-supplied column NAMES exist in ``sample.df``. Without
    # this, a typo like ``{'strata': 'stratum_id'}`` would silently build a
    # ``SurveyDesign`` referencing a non-existent column and only fail much
    # later inside diff-diff with a less actionable error message. Skip
    # ``_LITERAL_VALUE_DESIGN_FIELDS`` because their values are configuration
    # literals (e.g. ``weight_type="pweight"``, ``replicate_method="JK1"``,
    # ``fay_rho=0.5``, ``mse=True``), not column references on ``sample.df``.
    user_column_keys: dict[str, str] = {
        k: v for k, v in user.items() if k not in _LITERAL_VALUE_DESIGN_FIELDS
    }
    missing_user_cols: dict[str, str] = {
        k: v for k, v in user_column_keys.items() if v not in df_columns
    }
    if missing_user_cols:
        raise ValueError(
            "design_columns references column(s) that do not exist in "
            f"sample.df: {sorted(missing_user_cols.items())}. "
    ...
    #
    # Because of this opportunistic auto-population, a regular covariate
    # that happens to match a default name (e.g. a column literally called
    # ``"psu"`` that is NOT a sampling unit) WILL silently opt the
    # ``SurveyDesign`` into clustered/stratified variance handling. To make
    # that decision visible, we log every default that gets layered in --
    # users who do not want this behaviour should pass an explicit
    # ``design_columns`` mapping (which suppresses the defaults for the
    # supplied keys).
    ...
    for key, default_col in DEFAULT_DESIGN_COLUMNS.items():
        ...
        if default_col in df_columns:
            ...
            logger.info(
                ...
                "convention name). If this column is a regular covariate "
                "rather than a survey-design field, pass an explicit "
                "design_columns={...} that omits %r to suppress this.",
                key,
                default_col,
                key,
            )
